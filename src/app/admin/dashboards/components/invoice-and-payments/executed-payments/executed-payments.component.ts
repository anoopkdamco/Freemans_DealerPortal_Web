...
@Component({
  selector: 'app-executed-payments',
  templateUrl: './executed-payments.component.html',
  styleUrls: ['./executed-payments.component.scss']
})
export class ExecutedPaymentsComponent implements OnInit {
  ...
  public executedDataList: IPayments[] = [];
    ...
    .subscribe((data: any) => {
      if (data.objectLoading == 'Home_Records_Success') {
          ...
          for(let i=0; i< this.invoiceData.length; i++) {
            if(this.invoiceData[i].Payment_Status__c == 'Paid') {
              this.executedDataList.push(this.invoiceData[i]);
            }
        }
          for (const item of this.executedDataList) {
          ...
          this.dataSource = new MatTableDataSource(this.executedDataList);
 ...
 openOrderDialog(val) {
  const dialogRef = this.dialog.open(OrderDialogComponent, {width: '900px', data: val});
